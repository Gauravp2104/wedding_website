...
import { syncRsvpsToSheet } from './google-sheets.js';
 ...
 *
 * Every write also re-syncs the full RSVP list to a Google Sheet (see
 * lib/google-sheets.js) so the couple have a live, current view without
 * needing to touch the JSON.
 *
...
export async function appendRsvp(entry) {
  ...
  // Best-effort: the JSON above is the source of truth, so a Google Sheets
  // sync failure never fails the RSVP save.
  try {
